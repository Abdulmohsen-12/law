import 'dart:io';
import 'dart:math';
import 'package:law/Screens/input_section/consultion.dart';
...
import '../input_section/extraservice.dart';
...
class _OrderdetailsState extends State<Orderdetails> {
  ...
  Widget build(BuildContext context) {
    ...
    final TextEditingController _servicetype_editing_C =
                        ...
                        child: Column(
                          children: [
                            Padding(
                              ...
                              child: Service_name.contains("case-lawsuit")
                                  ? Container(
                                                      ...
                                                      children: [
                                                        ...
                                                        isdeletelick
                                                            ? GestureDetector(
                                                                onTap: () {
                                                                  setState(() {
                                                                    APIService.checkAndShowCircularDialog(
                                                                        context,
                                                                        true);
                                                                    deletefile(
                ...
                Padding(
                  ...
                  child: GestureDetector(
                      onTap: () {
                        showDialog(
                            context: context,
                            builder: (BuildContext context) => extra_dialog(
                                  docdescrip_editing_C: _servicetype_editing_C,
                                  date_editing_C: _amount_editing_C,
                                  label1: Constant.servicetype_txt,
                                  label2: Constant.amount_txt,
                                  titletext: Constant.extra_serv_txt,
                                ));
                      },
                      child:
                          const Whitebutton(btntext: Constant.extra_serv_txt)),
                ),
                Align(
                              ...
                              child: !isinfoclick
                                  ? Container(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Image.asset(
                                            "assets/icons/informationICOn.png",
                                            color: Colors.white,
                                            scale: 1.3,
                                          ),
                                          SizedBox(
  ...
  Future<void> getcasedetails() async {
    ...
    CaseDetailsResponse casedetials_response =
        caseDetailsResponseFromJson(_responseBody);
    if (casedetials_response.statusCode == 200) {
      ...
      setState(() {
        ...
        DateTime p =
            DateTime.parse(casedetials_response.data.createTime.toString());
        created = p.day.toString() +
            "/" +
            p.month.toString() +
            "/" +
            p.year.toString();
  ...
  Future<void> deletefile(String file_id) async {
    String _responseBody = await APIService.getdataa(
        data: file_id,
        Route: ApiRoute.file_deleted,
        context: context,
        show_CircularDialog: false);
    print(_responseBody);
    SucceededResponse succeededResponse =
        succeededResponseFromJson(_responseBody);
    if (succeededResponse.status_code == 200) {
      setState(() {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
              builder: (context) =>
                  DrawerScreen(place: Orderdetails(Order_Id: widget.Order_Id))),
        );
      });
    }
  }
  ...
  void actionPopUpItemSelected(String value, String name) {
    ...
    if (value == 'edit') {
      if (Service_name == "consultation") {
        showDialog(
            context: context,
            builder: (BuildContext context) => Consultion_Dialog(
                  update: true,
                  caseID: widget.Order_Id,
                ));
      }
      if (Service_name == "Contract Drafting") {
        showDialog(
            context: context,
            builder: (BuildContext context) => Contract_draft_Dialog(
                  update: true,
                  caseID: widget.Order_Id,
                ));
      }
      if (Service_name == "case-lawsuit") {
        showDialog(
            context: context,
            builder: (BuildContext context) => Lawsuit_Dialog(
                  update: true,
                  caseID: widget.Order_Id,
                ));
      }
